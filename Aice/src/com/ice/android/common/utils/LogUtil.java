...
/**
 * 日志记录工具类 可以通过修改是否启用调试模式、日志级别、是否记录日志到文件, 灵活的记录日志。<br/> 
 * 记录日志到文件需要配置权限：android.permission.WRITE_EXTERNAL_STORAGE<br/> 
 * 支持外部文件配置的方式进行日志的管理（配置文件详细可参考： /assets/log.properties）
 * @author kjxu
 * @version 
 */
public class LogUtil {
     ...
     * 日志文件名称绝对路径
     */
    private static String LOGFILENAME = Environment.getExternalStorageDirectory().getPath() + "/iss/app_name/log/app_name.log";
    ...
    static {
        try {
            ...
            p.load(LogUtil.class.getResourceAsStream("/assets/aiceConfig.properties"));

            LogUtil.d(TAG, "load config: " + p.toString());
            ...
            LogUtil.d(TAG, "ADB = " + ADB);
            LogUtil.d(TAG, "LOG_DEGREE = " + LOG_DEGREE);
            LogUtil.d(TAG, "LOGFILENAME = " + LOGFILENAME);
